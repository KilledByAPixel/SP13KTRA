...
// wavedash.js).
//
// Ported from the LittleJS Newgrounds plugin with three changes: no Medal class, fetch
// instead of synchronous XHR (the plugin's blocking calls at start freeze the page for a
// logged-in player), and the browser's crypto.subtle instead of CryptoJS for the AES-128
// encryption (the same secure field). Names read off an object here must stay out of
// build.js's MANGLE_PROPS, which lists get, add and name: no Map, no Set, and no name key
// in an object literal (build.js's Newgrounds name check).
///////////////////////////////////////////////////////////////////////////////

// from the project's API Tools on newgrounds.com, which assigns the medal and scoreboard ids
const ngAppId = '62781:SKEa7Fu7';

// the AES-128 / Base64 encryption key is BLANK HERE and must stay blank: it lives in
// newgrounds.key, which git ignores, and build.js writes it into the enhanced build's source
// by replacing this exact line. The dev page and the public page have no key, so their hooks
// stay off; tools/public-sync.js refuses to copy a game file that holds the key
const ngKey = '';

// medal ids in achievementsEarned order: Chequered Flag (finish a race), Redshift .. Race
// the Spectrum (a win on track 1 .. 8), Open Road (every circuit unlocked), Full Spectrum
// (a win on all), Supernova (secret: explode in a race)
const ngMedals = [92365, 92366, 92367, 92368, 92369, 92370, 92371, 92372, 92373, 92374, 92375, 92376];

// scoreboard ids per circuit, track 1 .. 8: Standard, Low to High, Time, in milliseconds
const ngBoards = [16186, 16187, 16188, 16189, 16190, 16191, 16192, 16193];

// the session, read at call time (a regex: URLSearchParams' get is in MANGLE_PROPS);
// falsy off Newgrounds, logged out or with no key
const ngSession = () => newgroundsMode && ngAppId && ngKey && (location.search.match(/[?&]ngio_session_id=([^&]*)/) || 0)[1];

// medal id: 1 once Newgrounds lists it unlocked or an unlock is on its way (a failed unlock clears it)
const ngUnlocked = {};
...
{
    if (!ngSession()) return;
    // the medals the player already has, so a start unlocks only what the save earned since;
...
{
    if (!ngSession()) return;
    // this race's time in milliseconds: the board ranks the lowest first
    const c = currentCircuit, board = ngBoards[c], score = Math.round(raceTime*1000);
...
// unlocks every medal earned that the player does not have yet
function ngAchieve()
{
    if (!ngSession()) return;
    ...
    {
        const id = ngMedals[i];
        // a failure is tried again on the next finish
        id && !ngUnlocked[id] && (ngUnlocked[id] = 1, ngCall('Medal.unlock', {id}).then(d => d || (ngUnlocked[id] = 0)));
    }
}

// one call through the Newgrounds.io gateway, encrypted as the plugin does: the call's JSON
// {component, parameters} under AES-128-CBC with a random 16-byte IV, the IV then the
// ciphertext in base64 as call.secure, parameters 0 in the clear. Resolves to the
// component's data on success, else 0 after a console warning; it never rejects
async function ngCall(component, parameters)
{
    try
    {
        ...
        // the algorithm from string pairs, not {name: 'AES-CBC', iv}: name is in MANGLE_PROPS,
        // and the renamed key makes every call in the enhanced build fail with "Algorithm:
        // name: Missing" (test/newgrounds-walk.js catches it; the Node test runs unmangled source)
