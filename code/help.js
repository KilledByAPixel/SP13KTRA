...
///////////////////////////////////////////////////////////////////////////////
// help.js - how to play: in-race tips until the first finish, and the menu's HELP card
//
// Dev page and enhanced build only: the 13k build never includes this file, and every call
// into it is behind enhancedMode. The wording follows the device in use now (helpDevice):
// the touch pad, a gamepad, the mouse, else the keyboard.
//
// Tips: one line at a time over the race HUD, each shown once per race for 4 s, while no
// circuit has been finished (bestPlaces all 0); a death retry shows them again. Triggers, in
// priority order: the countdown (the controls), 8 s in (the turbo), the first wall hit,
// energy under 60, the first pad, the first rough shoulder (vehicle.js's bumpDistance only
// changes on one) and 25 s in (lift to turn tighter).
//
// The HELP card is disabled (helpCard 0) and its code kept: a HELP word on the menu, opened
// by a click or tap on it, H or the gamepad's Y; the card lists the rules and the controls,
// and any key, click, tap or button closes it. Nothing calls drawHelpButton while it is off.
//
// Called from: hud.js drawHUD (drawHelpCard, drawHelpTips); game.js's menu (helpMenu, which
// takes the menu's input while the card is involved).
///////////////////////////////////////////////////////////////////////////////

const helpNames = {keys: 'KEYBOARD', mouse: 'MOUSE', pad: 'GAMEPAD', touch: 'TOUCH'};

// per device: the countdown tip, the turbo tip, the card's controls line
const helpTexts = {
    keys: ['UP GAS · ARROWS STEER · SPACE BRAKE', 'SHIFT: TURBO · IT USES ENERGY', 'UP/W GAS · ARROWS/A D STEER · SPACE/DOWN BRAKE · SHIFT TURBO · ESC MENU · M MUSIC'],
    ...
    pad: ['A GAS · STICK STEERS · X BRAKE', 'RB OR B: TURBO · IT USES ENERGY', 'STICK STEERS · A/RT GAS · X/LT BRAKE · RB/B TURBO'],
...
// 0: the HELP card is disabled (too busy, too much text); the in-race tips stay
const helpCard = 0;

// helpOpen: the card is showing
// helpBox: the HELP word's hit box {x0, x1 in mouseX units, y, s} as last drawn
let helpOpen = 0, helpBox = 0;

// helpRace: the race the tips belong to (its vehicles array); helpShown: the tips shown in it
// helpTip, helpTipTime: the tip showing and when it started; helpBump: the last bumpDistance seen
let helpRace, helpShown = {}, helpTip = 0, helpTipTime = 0, helpBump = 0;

const helpDevice = () => touchHud() ? 'touch' : isUsingGamepad ? 'pad' : mouseMode ? 'mouse' : 'keys';

// drawHUDText, shrunk to fit 92% of the width, so a long line never runs off a phone screen
...
// the menu's input while HELP is involved: true on a frame the card takes (open, or opening
// or closing it now), so the menu ignores that frame
function helpMenu()
{
    if (!helpCard) return 0; // disabled: no H, no Y, and hud.js draws no HELP word
    if (helpOpen)
    {
...
// the HELP word, on the TEAM row (row circuitCount): beside the word on the wide menu, at the
// right end of the row on a tall one; white, its shadow in the craft's colour under the pointer
// (not called while helpCard is 0)
function drawHelpButton()
{
    const tall = getAspect() < 1, y = menuRowY(circuitCount), s = menuRowSize(circuitCount)*(tall ? .4 : .5);
    const x = tall ? .95 : (menuRowW[circuitCount]+1)/2 + .05/getAspect(); // wide: the row's measured right edge plus a gap
...
{
    if (/[1-8]/.test(bestPlaces)) return helpTip = 0; // a finish anywhere turns the tips off for good
...
// a tip, centred. On a tall window one too wide for 92% of the width splits into two lines at
// the break nearest its middle, a · (dropped) if it has one, else a space: shrunk to fit on a
// portrait phone it is hard to read. Wide windows keep one line, and each line still shrinks
// to fit if it must (drawHelpText)
