...
//   updateCamera() the chase camera; vehicle.js also calls it to reseat after a respawn
//   writeSaveData() called on a menu pick here, and by vehicle.js when a race finishes
...
const random = new Random;     // the shared generator: the world build and the music bake seed it
let autoPause = enhancedMode;  // pause on focus loss: enhanced build only
let autoFullscreen = 0;

// setup
// laneWidth: the road half-width is laneWidth*1.6*laneCount (trackGen.js); pads sit on lane centres
const laneWidth = 700;
const trackSegmentLength = 100;    // route units per segment: s advances 100 per sample
const cameraBoomZ = 1200;          // the chase camera's distance behind the craft (updateCamera)
...
// route units per lap (400,000; world length is routeScale times that)
const lapDistance = lapTrackSegments*trackSegmentLength;
const raceLaps = 3;
const circuitCount = 8; // one per racer
let currentCircuit = 0;
let levelInfo; // the active circuit's LevelInfo (levels.js), pinned for the whole race
let playerLap, playerPlace;

const fieldSize = 8;   // craft in a race: the player and 7 rivals (from the front you never see more)
let lastRacePlace = fieldSize; // the last finishing place, for the results card (the grid ignores it)
...
let time, frame, frameTimeLastMS, frameTimeBufferMS, paused, focusPaused;
let startCountdown, gameOverTime;
let raceTime, playerWin;
let titleScreenMode = 1, menuMode = 0; // menuMode: the title's menu (circuit and craft select, hud.js)
let trackSeed = 1331; // the world's random seed: every circuit is preset

///////////////////////////////
// game variables

// cameraRoll: the bank lean in degrees, applied on the view matrix only (webgl.js)
let cameraPos, cameraRot, cameraRoll=0;
// vehicles[0] is the player at a race start (the menu's craft pick swaps the reference)
...
function gameInit()
{
    ...
    // no <style> tag at all (property sets pack smaller than the tag or one cssText string).
    // The enhanced shell adds its centring transform
    ...
    // the Wavedash platform (wavedash.js): once the game can draw, and before the first
    // frame, so a throw in that frame cannot leave the game behind the platform's loading
    // screen (init dismisses it; the frame still paints in this same task). Caught, so a
    // throw from the SDK can never stop gameUpdate from starting the loop (a black screen)
    if (wavedashMode)
...
function gameStart()
{
    ...
    // the craft-contact cooldowns and the sound timers (vehicle.js): the clock restarts at 0
    // each race, so a timer left over from the last race would fire late in this one
    contactTimes=[]; lowBeepTime=lapBeepTime=lapBeeps=0;
    ...
    musicLoad(); // a new circuit's loop from its seed; the same circuit carries on (music.js)
    playerPlace=fieldSize;

    // the grid is fixed: the player always takes the back slot and the rivals fill the rest
    // in order (their colour goes by grid order, their skill by colour: vehicle.js). One grid
    // for everyone keeps leaderboard times comparable; a grid by the last finishing place
    // hands a winner, or an old save, pole for ever, and the field then queues behind
    const slot=fieldSize-1;
    ...
    // attract mode: the field spread down the road a fifth of a lap in, for the camera to look at
    if(titleScreenMode)
    {
        // the player craft (vehicles[0]) starts at the back of the field, so it runs in the
        // pack: started in front, the rivals' traffic rule holds them behind it and it leads
        for(let i=0;i<vehicles.length;++i) vehicles[i].place(80000+i*750,slotX(i));
        // a warm-up, enhanced only (too big for the 13k build): three seconds of the attract
        // race run before the first frame, so a circuit picked in the menu opens on a field
        // already at speed with its trails, not parked in a line. The clock runs on from there
...
function gameUpdateInternal()
{
    if (titleScreenMode)
    {
        ...
        // where a click on a name or Up/Down (wrapping) picks the circuit from the unlocked
        // list, the TEAM button or Left/Right the craft, Space or a click on the selected name
        // races, and Escape returns to the title. Results return to the menu. The attract race
        // never restarts on its own: the field laps until a new circuit or a race.
        // enhanced: Enter and the gamepad's A or Start act as Space
        let go = keyWasPressed('Space') || enhancedMode && (keyWasPressed('Enter') || keyWasPressed('NumpadEnter') || isUsingGamepad && (gamepadWasPressed(0)||gamepadWasPressed(9)));
        // the free cam has the keys and the mouse (debug.js) and hides the HUD: the title and
        // the menu ignore them (the else below is the menu's)
        if (!freeCamMode)
        if (!menuMode)
            (go || mousePressed) && (menuMode = 1, sound_checkpoint.play(.5));
        // the help card takes the menu's input while it is open, and on the frame it opens
        // or closes (help.js)
        else if (!(enhancedMode && helpMenu()))
        {
            ...
            // Row circuitCount is the TEAM button; the enhanced menu adds PLAY (circuitCount+1)
            // and FULL SCREEN (circuitCount+2)
            let pick = 0, craft = 0;
            const c = mousePressed ? menuRowAt() : -1;
            if (c >= 0) enhancedMode && c > circuitCount + 1 ? toggleFullscreen() : enhancedMode && c > circuitCount ? go = 1 : c == circuitCount ? craft = 1 : c >= circuitsUnlocked() ? sound_bump.play(.5,.7) : c == currentCircuit ? go = 1 : pick = c - currentCircuit;
            if (go)
            {
                titleScreenMode = 0;
                gameStart();
            ...
            pick ||= keyWasPressed('ArrowDown') - keyWasPressed('ArrowUp'); // in every build
            // the left stick, latched, browses the menu too (dev and enhanced builds); the d-pad
            // is in it already (input.js copies it in: reading the buttons as well moves two rows)
            ...
            if (pick)
            {
                ...
                // (a reload of the same one looks like a glitch); the world rebuilds
            ...
            if (craft)
            {
                ...
                // the enhanced build has been keeping a camera behind this craft all along
                // (craftCameras), so the cut lands on one already in the moving steady state.
                // The 13k build re-seats with the craft frozen, which is close but not seated:
                // the camera drifts about 650 units after the cut, against about 40 kept
    ...
    else
    {
        ...
        {
            // go back to title screen; after a win the enhanced build advances like Space does
            // (the 13k build leaves Escape on the same circuit, the unlock and records already saved)
            enhancedMode && playerWin && (currentCircuit = (currentCircuit+1)%circuitCount, writeSaveData());
            sound_charge.play();
            titleScreenMode = 1;
            gameStart();
        }
        // (the R restart is in enhancedModeUpdate: a dev/enhanced key, not a 13k one)
...
{
    // asked for first: an exception anywhere in a frame then costs that frame, not the game
    requestAnimationFrame(gameUpdate);
    if (!clampAspectRatios)
        // the 13k build: the canvas is the window
    ...
    if (enhancedMode)
    {
        if (paused)
        {
            ...
            inputUpdate();
            // the fixed step is skipped: stop the engine loop here (updateCars restarts it)
            if (engineSound) engineSound.stop(), engineSound = 0;
            ...
            // the dev free camera still flies while the game is frozen (debug.js)
            debug && freeCamMode && debugPausedUpdate(frameTimeMS - frameTimeLastMS);
            inputUpdatePost();
        }
    }
    ...
    {
        // held + runs time ten times faster, held - a tenth
    ...
    // slow framerate: three catch-up steps at most, then the game slows rather than spirals.
    // 42, not 50: after any stall, every gameStart included, 50 runs exactly three steps and
    // leaves the buffer on the one-or-two-steps boundary, where 60 Hz frame times rounded to
    // .1 ms freeze and double a third of the frames; 42 leaves it mid-frame
    frameTimeBufferMS = min(frameTimeBufferMS, 42);
    ...
    for (;frameTimeBufferMS >= 0; frameTimeBufferMS -= 1e3/frameRate)
    {
        ...
        musicMuted ^= keyWasPressed('KeyM'); // the music on or off, every build; the sound effects stay
        debug && debugUpdate();
        inputUpdate();
        musicUpdate(); // the one music loop (music.js): starts, stops and retries by state and focus

        if (enhancedMode && (!titleScreenMode || debug && freeCamMode)) // the free cam freezes the title's field too
        ...
        // before the real camera, so it leaves the globals as it found them
        enhancedMode && titleScreenMode && menuMode && updateCraftCameras();
        updateCamera();
...
function enhancedModeUpdate()
{
    ...
    // F toggles fullscreen from anywhere, the title, the menu and a race alike (the menu's
    // FULL SCREEN row is the same toggle for a mouse or a thumb). In dev mode F is the free
    // camera, so the dev page keeps that until dev() is off
...
// enhanced only: while the menu is up, keep a camera behind every selectable craft as if it
// were the player's, so changing team cuts to one already in the moving steady state
// (re-seating with the craft frozen lands close but not seated, and the camera then slides
// into place over half a second). updateCamera works on the globals, so they are swapped
// around each craft's run and put back
let craftCameras = [];
...
function updateCamera()
{
    ...
    // sideways slip shows the craft swung across the screen rather than the world spinning
    // (below 1000 units/s the velocity direction is noise: use the heading)
    ...
    // the camera's own forward on the ground plane; the target is a boom length back along
    // it and 900 units up
    const f=vec3(Math.sin(cameraRot.y),0,Math.cos(cameraRot.y));
    const target=v.pos.subtract(f.scale(cameraBoomZ)).addSelf(vec3(0,900));
    ...
    // look down .26 rad plus half the road's pitch (positive pitch = descending, track.js)
    cameraRot.x=lerp(.12,cameraRot.x,.26+info.pitch*.5);
    // roll with a third of the road's bank, in degrees (19 of 57.3). Never cameraRot.z:
    // buildMatrix turns that about world Z after the yaw (right facing +Z, the wrong way
    // facing -Z, a nod facing +-X); glPreRender rolls the view matrix in camera space
    cameraRoll=lerp(.08,cameraRoll,-Math.atan(info.roll)*19);
    ...
    // the dev map view (debug.js): 500k straight up over the loop's centre plus the pan;
    // glPreRender goes orthographic
    if(topDownMode)
    {
        cameraPos=trackMapCenter.add(topDownPan).addSelf(vec3(0,5e5,0));
        cameraRot=vec3(PI/2,0,0); cameraRoll=topDownRotate?90:0; // pitch only: vec3(s) is (s,s,s); X turns the view a quarter
    }
}
...
// the best placing per circuit as one digit each (0 = never finished), then each circuit's
// best finishing time in seconds (0 or empty = none). The menu shows the placing with the
// time under it and browses up to the circuit after the last one finished. Older SP13K
// saves are ignored. Read once at load, here, after the declarations above; written on a
// circuit or craft pick and on a finish (vehicle.js)
const saveName = 'SP13KTRA';

// the enhanced build survives denied storage (a SecurityError would stop the page before
// gameInit), no progress kept; the 13k build folds to the plain read and write
...
// (the finishing place, field 1, is still written so the positional save keeps its shape,
// but nothing reads it back: the grid is fixed)
playerCraft = mod(saveData[2]*1 || 0, 6);
let bestPlaces = (saveData[3] || '').padEnd(circuitCount, 0); // a string of circuitCount digits
// musicMuted: the M key's music-off flag (not saved)
// bestTimes[c]: the best finish on circuit c in seconds, 0 for none
let musicMuted = 0, bestTimes = saveData.slice(4).map(Number);
// every circuit up to the one after the last finished
const circuitsUnlocked = () => min(circuitCount, bestPlaces.search(/0*$/)+1);

function writeSaveData()
{
    // toString joins them (a hole in bestTimes joins as nothing and reads back as 0). The
    // array is spelled out in both branches so the 13k build folds to exactly the plain write
    // (a shared const survives the fold)
    // (enhanced: storage denied or full, the session plays on unsaved)
    if (enhancedMode)
        try { localStorage[saveName] = [currentCircuit, lastRacePlace, playerCraft, bestPlaces, ...bestTimes]; } catch(e) {}
    else
